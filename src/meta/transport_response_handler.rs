...
use crate::nebula_fbthrift_meta_v3::services::meta_service::{
    GetPartsAllocExn, GetSpaceExn, ListEdgesExn, ListHostsExn, ListPartsExn, ListSpacesExn,
    ListTagsExn,
};
...
impl ResponseHandler for MetaTransportResponseHandler {
    fn try_make_static_response_bytes(
        ...
        match fn_name {
            ...
            | b"MetaService.getPartsAlloc"
            | b"MetaService.listHosts" => Ok(None),
            _ => Err(IoError::new(
                IoErrorKind::Other,
        ...
        match message_type {
            MessageType::Reply => {
                match &name[..] {
                    ...
                    b"listHosts" => {
                        let _: ListHostsExn = match Deserialize::read(&mut des) {
                            Ok(v) => v,
                            Err(_) => return Ok(None),
                        };
                    }
                    ...
                    b"getPartsAlloc" => {
                        let _: GetPartsAllocExn = match Deserialize::read(&mut des) {
                            Ok(v) => v,
                            Err(_) => return Ok(None),
                        };
                    }
